...
data_points = []
running = True
clf = joblib.load("RF_classifier.pkl") # Handcrafted multi-class model
...
first = 1
# Global to hold the main root window
root = None
...
def trigger_detection():
    ...
    if not detection_running:
        detection_running = True  # Start detection loop

    print("Detection running...")  # Replace with real logic here
    countdown_and_predict(0)
...
def perform_prediction():
    ...
    if len(buffer) >= MIN_WINDOW_SIZE:
        ...
        print("prediction_label updated %s" % prediction)
